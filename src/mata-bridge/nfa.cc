...
Nfa& Nfa::revert() {
    auto tmp{ mata::nfa::revert(nfa_impl)};
    std::swap(nfa_impl, tmp);
    return *this;
}

Nfa& Nfa::minimize_brzozowski() {
    auto tmp{ mata::nfa::algorithms::minimize_brzozowski(nfa_impl) };
    std::swap(nfa_impl, tmp);
    return *this;
}

Nfa& Nfa::minimize_hopcroft() {
    auto tmp{ mata::nfa::algorithms::minimize_hopcroft(nfa_impl) };
    std::swap(nfa_impl, tmp);
    return *this;
}
...
Nfa& Nfa::reduce_simulation() {
    ...
    auto tmp{ mata::nfa::algorithms::reduce_simulation(nfa_impl, reduced_state_map) };
    std::swap(nfa_impl, tmp);
    return *this;
}
...
Nfa& Nfa::reduce_residual(const std::string& type, const std::string& direction) {
    std::unordered_map<State,State> reduced_state_map;
    auto tmp{ mata::nfa::algorithms::reduce_residual(nfa_impl, reduced_state_map, type, direction) };
    std::swap(nfa_impl, tmp);
    return *this;
}
...
Nfa& Nfa::union_det_complete(const Nfa& aut) {
    auto tmp{ mata::nfa::union_det_complete(nfa_impl, aut.nfa_impl) };
    std::swap(nfa_impl, tmp);
    return *this;
}

Nfa& Nfa::determinize() {
    auto tmp{ mata::nfa::determinize(nfa_impl) };
    std::swap(nfa_impl, tmp);
    return *this;
}

Nfa& Nfa::intersection(const Nfa& aut, const Symbol first_epsilon) {
    auto tmp{ mata::nfa::intersection(nfa_impl, aut.nfa_impl, first_epsilon) };
    std::swap(nfa_impl, tmp);
    return *this;
}

Nfa& Nfa::complement_classical(const OrdVector<Symbol>& symbols) {
    auto tmp{ mata::nfa::algorithms::complement_classical(nfa_impl, symbols) };
    std::swap(nfa_impl, tmp);
    return *this;
}

Nfa& Nfa::complement_brzozowski(const OrdVector<Symbol>& symbols) {
    auto tmp{ mata::nfa::algorithms::complement_brzozowski(nfa_impl, symbols) };
    std::swap(nfa_impl, tmp);
    return *this;
}
